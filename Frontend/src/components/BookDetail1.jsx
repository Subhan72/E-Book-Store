...
const BookDetail = ({ book }) => {
  const handleAddToWishlist = async () => {
    try {
      ...
      // Ensure the book object has the required fields before sending
      const bookDetails = {
        ISBN: book.ISBN,
        title: book.title,
        author: book.author,
      ...
      if (!bookDetails.ISBN || !bookDetails.title || !bookDetails.author || !bookDetails.genre) {
      ...
      const response = await axios.post(
        ...
        { bookDetails }, // Send the object as 'bookDetails'
        {
          headers: {
            applicatiom: JSON,
  ...
  const handleAddToCart = async () => {
    try {
      ...
      console.log('Sending data:', { bookId: book._id, quantity: 1 });
  
      const response = await axios.post(
        'http://localhost:3000/add-to-cart',
        { bookId: book._id, quantity: 1 }, // Only send bookId and quantity
        {
          headers: {
            Authorization: `${token}`,
          },
